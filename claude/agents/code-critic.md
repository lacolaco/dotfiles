...
**Secure by Design**: Security is a property of the design, not a checklist applied late. Defaults must be safe (closed > open, deny > allow, least-privilege > broad scope). Trust boundaries must be explicit and validated at the boundary—untrusted input parsed into typed values, never threaded raw through the system. Failure modes must fail closed, not silently continue past auth/crypto errors. Secrets belong in secret stores, never in code, logs, error messages, or client bundles. Cryptographic primitives must come from vetted libraries used correctly (no hand-rolled hashing, no ECB, no predictable IVs, constant-time comparison for secrets). When a security-critical decision relies on per-PR human vigilance, the design is wrong—structural fixes that eliminate the bug class beat reviewer checklists.
